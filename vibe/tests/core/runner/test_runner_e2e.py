...
def test_runner_basic_file_modification(complex_git_repo, mock_inquirer_accept_all):
    ...
    assert "src/main.py" in log_output or "Modify" in log_output or "Update" in log_output
...
def test_runner_multiple_files_complex_changes(complex_git_repo, mock_inquirer_accept_all):
...
def test_runner_with_file_deletion_and_addition(complex_git_repo, mock_inquirer_accept_all):
...
def test_runner_with_file_rename(complex_git_repo):
    ...
    subprocess.run(["git", "mv", "src/utils.py", "src/utilities.py"], cwd=repo_path, check=True)
...
def test_runner_content_based_grouping(complex_git_repo, mock_inquirer_accept_all):
    ...
    git_interface = SubprocessGitInterface(repo_path)
    chunker = DeterministicChunker()
    grouper = DeterministicGrouper(group_by_file=False, max_chunks_per_group=2)  # Content-based grouping
...
def test_runner_chunking_and_grouping_integration(complex_git_repo, mock_inquirer_accept_all):
...
"""
    ...
    git_interface = SubprocessGitInterface(repo_path)
    chunker = DeterministicChunker(split_keywords=['SPLIT_HERE'])  # Will split at SPLIT_HERE
    grouper = DeterministicGrouper(group_by_file=True, max_chunks_per_group=1)  # One chunk per group
