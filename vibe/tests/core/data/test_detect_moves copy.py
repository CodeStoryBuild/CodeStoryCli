...
from vibe.core.data.utils import detect_moves
...
def test_detect_moves_no_moves_only_additions():
    ...
    assert len(out) == 2
    assert (
        isinstance(out[0], Addition)
        and out[0].content == "line A"
        and out[0].line_number == 1
    )
    assert (
        isinstance(out[1], Addition)
        and out[1].content == "line B"
        and out[1].line_number == 2
    )
...
def test_detect_moves_no_moves_only_removals():
    ...
    assert len(out) == 2
    assert (
        isinstance(out[0], Removal)
        and out[0].content == "line A"
        and out[0].line_number == 1
    )
    assert (
        isinstance(out[1], Removal)
        and out[1].content == "line B"
        and out[1].line_number == 2
    )
...
def test_detect_moves_intervening_changes():
    ...
    assert (
        isinstance(out[0], Addition)
        and out[0].content == "new line"
        and out[0].line_number == 2
    )
    assert (
        isinstance(out[1], Removal)
        and out[1].content == "old line"
        and out[1].line_number == 3
    )
    assert (
        isinstance(out[2], Move)
        and out[2].content == "line A"
        and out[2].from_line == 1
        and out[2].to_line == 4
    )
...
def test_detect_moves_content_change_not_a_move():
    ...
    assert (
        isinstance(out[0], Removal)
        and out[0].content == "original content"
        and out[0].line_number == 1
    )
    assert (
        isinstance(out[1], Addition)
        and out[1].content == "modified content"
        and out[1].line_number == 2
    )
...
def test_detect_moves_same_line_add_remove_not_a_move():
    ...
    assert (
        isinstance(out[0], Removal)
        and out[0].content == "old"
        and out[0].line_number == 1
    )
    assert (
        isinstance(out[1], Addition)
        and out[1].content == "new"
        and out[1].line_number == 1
    )
...
def test_detect_moves_multiple_removals_one_addition_match_first_by_line():
    ...
    assert (
        isinstance(out[0], Removal)
        and out[0].content == "another line"
        and out[0].line_number == 2
    )
    assert (
        isinstance(out[1], Removal)
        and out[1].content == "test"
        and out[1].line_number == 3
    )
    assert (
        isinstance(out[2], Move)
        and out[2].content == "test"
        and out[2].from_line == 1
        and out[2].to_line == 4
    )
...
def test_detect_moves_multiple_additions_one_removal_match_first_by_line():
    ...
    """
    input = [Removal(1, "test"), Addition(2, "test"), Addition(3, "test")]
    out = detect_moves(input)
    assert len(out) == 2  # One move, one addition
    # Sorted by line_number (to_line for move)
    assert (
        isinstance(out[0], Move)
        and out[0].content == "test"
        and out[0].from_line == 1
        and out[0].to_line == 2
    )
    assert (
        isinstance(out[1], Addition)
...
def test_detect_moves_complex_interleaved_operations():
    ...
    assert (
        isinstance(out[0], Addition)
        and out[0].content == "new line 1"
        and out[0].line_number == 2
    )
    assert (
        isinstance(out[1], Addition)
        and out[1].content == "new line 2"
        and out[1].line_number == 4
    )
    assert (
        isinstance(out[2], Move)
        and out[2].content == "line A"
    ...
    assert (
        isinstance(out[3], Removal)
        and out[3].content == "line C"
        and out[3].line_number == 6
    )
    assert (
        isinstance(out[4], Move)
        and out[4].content == "line B"
        and out[4].from_line == 3
        and out[4].to_line == 7
    )
...
def test_detect_moves_unmatched_removal():
    ...
    input = [Removal(1, "line A"), Addition(2, "line B")]  # No matching addition
    out = detect_moves(input)
    assert len(out) == 2
    assert (
        isinstance(out[0], Removal)
...
def test_detect_moves_unmatched_addition():
    ...
    input = [Addition(1, "line A"), Removal(2, "line B")]  # No matching removal
    out = detect_moves(input)
    assert len(out) == 2
    assert (
        isinstance(out[0], Addition)
