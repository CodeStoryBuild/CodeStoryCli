...
def test_detect_replacements_simple_replacement():
    """Detect a single replacement."""
    input_changes = [Removal(1, "old line content"), Addition(1, "new line content")]
...
def test_detect_replacements_mixed_changes():
    ...
    assert len(output) == 6
    assert (
        isinstance(output[0], Replacement)
        and output[0].old_content == "old_A"
        and output[0].new_content == "new_A"
        and output[0].line_number == 1
    )
    assert (
        isinstance(output[1], Addition)
        and output[1].content == "standalone_add"
        and output[1].line_number == 2
    )
    assert (
        isinstance(output[2], Replacement)
        and output[2].old_content == "old_B"
        and output[2].new_content == "new_B"
        and output[2].line_number == 3
    )
    assert (
        isinstance(output[3], Move)
        and output[3].content == "moved_line"
        and output[3].to_line == 4
    )  # line_number is to_line for Move
    assert (
        isinstance(output[4], Removal)
        and output[4].content == "standalone_removal"
        and output[4].line_number == 5
    )
    assert (
        isinstance(output[5], Removal)
        and output[5].content == "old_C"
        and output[5].line_number == 6
    )
...
def test_detect_replacements_replacement_with_same_content():
    ...
    input_changes = [Removal(1, "same content"), Addition(1, "same content")]
    output = detect_replacements(input_changes)
    assert len(output) == 1
    assert isinstance(output[0], Replacement)
...
def test_detect_replacements_multiple_removals_then_addition():
    ...
    """
    input_changes = [Removal(1, "old 1"), Removal(2, "old 2"), Addition(3, "new")]
    output = detect_replacements(input_changes)
    assert len(output) == 3
...
def test_detect_replacements_removal_then_addition_then_other():
    ...
    assert (
        isinstance(output[0], Replacement)
        and output[0].old_content == "R1"
        and output[0].new_content == "A1"
    )
...
def test_detect_replacements_last_element_removal():
    ...
    input_changes = [Addition(1, "add"), Removal(2, "remove")]
    output = detect_replacements(input_changes)
    assert len(output) == 2
    assert isinstance(output[0], Addition)
    assert isinstance(output[1], Removal)
