...
class AIGitPipeline:
    ...
    def run(self, target : str) -> List[CommitResult]:
        ...
        track = inquirer.confirm(f"Would you like to track all untracked files within {target}?", default=False)
        ...
        for chunk in raw_diff:
            w_move = models.detect_moves(chunk.ai_content)
            w_replacements = models.detect_replacements(w_move)
            print(models.format_diff_for_llm(chunk.file_path, w_replacements))
