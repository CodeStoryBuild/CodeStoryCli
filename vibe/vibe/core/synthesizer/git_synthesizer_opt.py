...
class GitSynthesizer:
    ...
    def _create_blobs_batch(self, path_to_tempfile_map: Dict[str, Path]) -> Dict[str, str]:
    ...
    def _build_tree_from_plan(
        ...
        try:
            ...
            for file_path, chunks in chunks_by_file.items():
                ...
                chunks_data = [cd for chunk in chunks for cd in chunk.get_chunk_application_data()]
                
                # Use streaming apply, which returns a path to a temp file
                new_content_temp_path = self._apply_changes_hybrid(original_content, chunks_data)
