...
class StandardDiffChunk(DiffChunk):
    ...
    def split(self, split_indices: List[int]) -> List['DiffChunk']:
        ...
        boundary_points = sorted(list(set([0] + split_indices + [len(self.parsed_content)])))
    ...
    def extract(self, start: int, end: int) -> Optional['DiffChunk']:
        ...
        sub_removals = [item for item in sub_parsed_content if isinstance(item, Removal)]
        sub_additions = [item for item in sub_parsed_content if isinstance(item, Addition)]
    ...
    def extract_by_lines(self, start_line: int, end_line: int) -> Optional['DiffChunk']:
        ...
        sub_parsed_content = [
            item for item in self.parsed_content
        ...
        content_lines = [("+" if isinstance(i, Addition) else "-") + i.content for i in sub_parsed_content]
        content_str = "\n".join(content_lines)
    ...
    def from_hunk(cls, hunk: "HunkWrapper") -> "StandardDiffChunk":
        ...
        for line in hunk.hunk_lines:
            if line.startswith('+'):
                parsed_content.append(Addition(content=line[1:], line_number=current_new_line))
                current_new_line += 1
            elif line.startswith('-'):
                parsed_content.append(Removal(content=line[1:], line_number=current_old_line))
                current_old_line += 1
