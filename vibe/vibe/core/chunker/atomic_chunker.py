...
class AtomicChunker(MechanicalChunker):
        ...
        for chunk in diff_chunks:
            if isinstance(chunk, DiffChunk):
                mechanical_chunks.extend(
                    self.group_by_predicate(
                        self.is_whitespace, chunk.split_into_atomic_chunks()
                    )
                )
            else:
    ...
    def group_by_predicate(
        self, line_predicate, chunks: list[DiffChunk]
    ) -> list[Chunk]:
        n = len(chunks)
        i = 0
            ...
            if combined_indices:
                ...
                if len(combined_indices) > 1:
                    group_chunk = CompositeDiffChunk(
                        list(chunks[idx] for idx in combined_indices)
                    )
