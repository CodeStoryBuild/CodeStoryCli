...
from dslate.core.data.immutable_hunk_wrapper import ImmutableHunkWrapper
...
def git_repo(tmp_path: Path):
    """
    A pytest fixture that creates a temporary directory, initializes a Git
    repository in it, and configures user identity. It changes the current
    working directory to the new repo for the duration of the test.
    """
...
class TestGitDiffParsing:
    ...
    def test_parse_rename_with_modification(self, git_repo: Path):
        ...
        new_hash = self.git_commands.get_current_base_commit_hash()
        hunks = self.git_commands.get_full_working_diff(
