...
def test_read_new_content():
    ...
    mock_git.run_git_text_out.assert_called_once_with(["cat-file", "-p", "head:path/to/file.txt"])
...
def test_read_old_content():
    ...
    mock_git.run_git_text_out.assert_called_once_with(["cat-file", "-p", "base:path/to/file.txt"])
