...
class SemanticAnalyser:
    ...
    def _get_or_create_context(self, file_path: str, language: str) -> AnalysisContext:
        ...
        if parser and language in self.language_config:
            ...
            if old_content:
                ...
                context.before_scope_tree, context.before_symbol_table, context.before_all_scopes = self._build_context(ast, lang_conf)
            ...
            if new_content:
                ...
                context.after_scope_tree, context.after_symbol_table, context.after_all_scopes = self._build_context(ast, lang_conf)

        self.context_cache[file_path] = context
        return context

    def group_chunks_for_file(self, chunks: list[Chunk], file_content: str, language: str) -> list[list[Chunk]]:
        """
        ...
        self._annotate_analysed_hunks(analysed_hunks, ast_root, scope_root, symbol_table, lang_conf)

        # Step 4: Group the individual hunks semantically
        hunk_groups = self._group_analysed_hunks(analysed_hunks, list(all_scopes.keys()))
    ...
    def _build_context(self, root_node: Node, config: dict):
        ...
        scope_tree_root = Scope(id="module", type="module", name=None, start_line=1, end_line=root_node.end_point[0] + 1)
        symbol_table = {}
        ...
        def traverse(node, current_scope):
            ...
            if node_type in config.get("scopes", []):
                ...
                new_scope = Scope(id=scope_id, type=node_type, name=scope_name, start_line=start_line, end_line=end_line, parent=current_scope)
                current_scope.children.append(new_scope)
                all_scopes[scope_id] = new_scope
                
                if scope_name:
                    symbol = Symbol(name=scope_name, type=node_type, definition_scope_id=scope_id)
    ...
    def _annotate_chunks(self, chunks: list[Chunk], root_node: Node, scope_root: Scope, symbol_table: dict, config: dict):
    ...
    def _find_lca_scope(self, chunk: Chunk, scope_root: Scope) -> Scope:
        ...
        def find_innermost(scope: Scope):
            for child in scope.children:
                if child.start_line <= chunk.new_start and child.end_line >= chunk.new_end:
                    return find_innermost(child)
            return scope
        return find_innermost(scope_root)

    def _analyze_chunk_for_usages(self, chunk: Chunk, node: Node, symbol_table: dict, config: dict):
    ...
    def _group_chunks(self, chunks: list[Chunk], all_scope_ids: list[str]) -> list[list[Chunk]]:
...
if __name__ == "__main__":
    ...
    file_diff = git_commands.get_file_diff_with_renames(fileA="fileA.py", fileB="fileB.py")
