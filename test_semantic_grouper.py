...
from vibe.semantic_grouping.semantic_grouper.semantic_grouper import SemanticGrouper
...
def create_test_chunks():
    ...
    chunk2 = DiffChunk(
        ...
        file_mode="100644",
        parsed_content=[
            Addition(content="        # Added comment", line_number=8)
        ],
        old_start=7,
        new_start=8
    )
...
def main():
    ...
    query_manager = QueryManager("vibe/semantic_grouping/language_config/language_config.json")
    ...
    try:
        ...
        for i, group in enumerate(semantic_groups):
            print(f"  Group {i + 1}: {len(group.chunks)} chunks, fallback={group.is_fallback_group}")
